...
class Runner {
  ...
  async lintAll () {
    ...
    const exec = await this.container.exec(this._templateCommand(this.config.lint.cmd));
  ...
  async run (snippet) {
    ...
    for (const e of expected) {
      ...
      if (match === null) {
        ...
        if (match !== null) {
          throw new TestError('ERR_ORDER', `"${e}" found after ${previous}`);
        }
